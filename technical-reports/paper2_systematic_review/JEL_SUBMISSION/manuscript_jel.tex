...
\pagestyle{fancy}
\lhead{\small Sentiment, Narratives, and Economic Outcomes}
\rhead{\thepage}
\cfoot{}

\title{\textbf{Sentiment, Narratives, and Economic Outcomes:\\
Unresolved Questions and a Research Agenda}}
\author{} % Blind review
\date{}
...
\begin{abstract}
Despite 18 years of research on sentiment-based economic forecasting, three fundamental questions remain unresolved. First, does sentiment cause economic outcomes or merely predict them? The literature overwhelmingly relies on Granger causality---which identifies temporal ordering, not causal effects---and no study satisfies criteria for causal inference. We formalize the identification problem and show the field's central claim rests on correlational evidence. Second, is sentiment information or noise? The same empirical patterns (prediction with reversal) support both interpretations; the field lacks a formal theory specifying when sentiment reflects fundamentals versus transient overreaction. Third, whose economy is being studied? Geographic coverage is heavily concentrated (56\% USA, 84\% English, 0\% Africa/Latin America), raising fundamental questions about external validity. Our meta-analysis of 66 studies (2007--2025) establishes median 20\% RMSE improvement but reveals that 35\% of papers likely inflate estimates through real-time data violations. We propose a unified research program centered on the Bangla Economic Narrative Index as a blueprint, with testable hypotheses (H1--H4), causal identification strategies, and a phased 10-year agenda spanning geographic expansion, theoretical integration, and central bank adoption.
\end{abstract}

\noindent \textbf{Keywords:} Sentiment analysis; Economic narratives; Causality; Information vs. noise; External validity; Research agenda \\
\textbf{JEL Codes:} C53, C55, E37, E44, G12, G17

\section{Introduction}

Three unresolved questions define the frontier of sentiment-based economic forecasting. First, does sentiment cause economic outcomes or does it merely predict them? After 66 empirical studies, the literature's central claim---that media sentiment drives GDP, inflation, and asset prices---rests on Granger causality, which identifies temporal order, not causal effects. Second, is the information in sentiment genuine or noise? The same empirical pattern (prediction followed by reversal) has supported diametrically opposed interpretations for 18 years. Third, whose economy does sentiment forecasting describe? With 56\% of studies focused on the United States and 84\% analyzing English text, the evidence base excludes precisely those settings where sentiment indices could be most valuable.

This review organizes 18 years of research around these three tensions, providing three contributions for economists.

\textbf{First, we formalize the identification problem}. The literature has demonstrated robust correlations between sentiment and economic outcomes---median 20\% RMSE improvement across 66 studies---but has not established whether these correlations reflect causal mechanisms or omitted variable bias. We present a formal treatment (Section~3.4) showing that the condition $\mathbb{E}[\varepsilon_t \mid S_t] \neq 0$ is violated in essentially every study reviewed, and propose identification strategies (IV, DiD, RDD) that could resolve the question.

\textbf{Second, we quantify what we do not know}. Our meta-analysis of 66 studies provides best available estimates of sentiment's predictive power: 20\% median RMSE improvement (range 5--42\%), varying by domain and method. We document a critical validation gap---only 33\% of studies follow real-time protocols---and estimate that approximately 35\% of published papers inflate improvements by 1--3\%. We further identify the absence of any causal identification, any formal theory of when sentiment is information versus noise, and any evidence for most of the world's economies.

\textbf{Third, we propose a unified research program}. We convert the Bangla Economic Narrative Index from a suggestion to a concrete research proposal with testable hypotheses (H1--H4), explicit causal identification strategies, and a phased 10-year agenda. The program addresses all three tensions simultaneously: geographic expansion (hundreds of languages), causal identification (natural experiments in new markets), and the information-versus-noise question (cross-market comparisons).

The paper proceeds as follows. Section~2 establishes the theoretical mechanisms linking narratives to economic outcomes. Section~3 presents the evidence base---meta-analytic results, methodological distribution, and validation audit---alongside the formal identification problem. Section~4 documents geographic and linguistic coverage. Section~5 evaluates validation practices and develops a tiered framework. Section~6 proposes the BENI research program with testable hypotheses. Section~7 outlines a 10-year agenda. Section~8 concludes.
...
\subsection{Formalizing the Identification Problem}

The empirical specification common to most reviewed studies takes the form:

\begin{equation}
Y_{t+h} = \alpha + \beta S_t + \gamma' X_t + \varepsilon_{t+h}
\label{eq:causal}
\end{equation}

where $Y_{t+h}$ is the economic outcome at horizon $h$, $S_t$ is the sentiment index at time $t$, $X_t$ is a vector of lagged economic controls, and $\varepsilon_{t+h}$ is the error term. The reported finding---that $\hat{\beta} > 0$ (or $<0$) and statistically significant---is interpreted as evidence that sentiment predicts outcomes. However, consistent estimation of $\beta$ requires $\mathbb{E}[\varepsilon_{t+h} \mid S_t, X_t] = 0$. This condition fails if:

\begin{equation}
\mathbb{E}[\varepsilon_{t+h} \mid S_t, X_t] = \mathbb{E}[\varepsilon_{t+h} \mid \text{news}_t, X_t] \neq 0
\label{eq:endog}
\end{equation}

The error term $\varepsilon_{t+h}$ captures all determinants of $Y_{t+h}$ not included in $X_t$. If media sentiment $S_t$ responds to the same economic forces that affect $Y_{t+h}$---for instance, journalists write pessimistic stories because they observe deteriorating economic conditions that official statistics have not yet captured---then $S_t$ is correlated with $\varepsilon_{t+h}$ and $\hat{\beta}$ is biased. The direction of bias depends on the information structure: if sentiment reacts to fundamentals faster than official statistics, $\hat{\beta}$ overstates the causal effect; if sentiment systematically over- or under-reacts, bias could go either direction.

No study in the reviewed literature addresses this endogeneity. The path forward requires one of:

\noindent\textbf{Instrumental variables}: Find $Z_t$ such that $\text{Cov}(Z_t, S_t) \neq 0$ (relevance) and $\text{Cov}(Z_t, \varepsilon_{t+h}) = 0$ (exclusion). Potential instruments include media ownership changes, newspaper strikes, or platform shutdowns that shift sentiment exogenously.

\noindent\textbf{Difference-in-differences}: Compare outcomes across regions or time periods where sentiment moves for exogenous reasons (e.g., differential media coverage of identical economic fundamentals).

\noindent\textbf{Regression discontinuity}: Exploit thresholds in media attention (e.g., coverage triggered by economic statistics crossing politically salient thresholds).

\noindent\textbf{The do-operator}: Following Pearl (2009), the quantity of interest is $P(Y \mid do(S = s))$---the distribution of outcomes when sentiment is \emph{set} to $s$---not $P(Y \mid S = s)$---the distribution when sentiment is \emph{observed} to equal $s$. The literature estimates the latter; policy relevance requires the former. Until the condition $\mathbb{E}[\varepsilon_{t+h} \mid S_t, X_t] = 0$ is established through exogenous variation, the literature's central finding is correlation, not causation.
...
\section{Toward a Unified Research Program}

The three tensions identified above---causality, information versus noise, and geographic external validity---are not independent. They share a root cause: the field has grown empirically rather than programmatically. Studies accumulate method-by-method, domain-by-domain, language-by-language, without an overarching framework connecting them. We propose a unified research program that addresses all three tensions simultaneously.

\subsection{The Bangla Economic Narrative Index as a Blueprint}

The Bangla Economic Narrative Index (BENI) is a concrete proposal for how a single research project can advance all three frontiers simultaneously. BENI would construct the first native-language economic sentiment index for South Asia, targeting Bangladesh's \$460 billion economy (265 million Bangla speakers).

\hspace*{0.5em}\textbf{Hypothesis 1 (Predictive Power):} Bangla news sentiment extracted via BanglaBERT and topic modeling predicts Bangladesh GDP growth, CPI inflation, and exchange rate movements at 1--3 month horizons, improving RMSE by 15--25\% relative to ARIMA benchmarks. This would replicate the established 20\% median finding in a novel linguistic and economic context, testing whether the meta-analytic result generalizes.

\hspace*{0.5em}\textbf{Hypothesis 2 (Method Comparison):} BanglaBERT-based sentiment outperforms translated English BERT for predicting Bangladesh economic outcomes by 5--10 percentage points RMSE improvement. This tests whether native-language models capture sentiment that translation destroys, simultaneously addressing the information-versus-noise question: if translation degrades predictive power, sentiment conveys linguistically specific information, not noise.

\hspace*{0.5em}\textbf{Hypothesis 3 (Causal Identification):} Exogenous variation in Bangla news sentiment---identified through natural experiments such as newspaper strikes, ownership changes, or platform shutdowns---causally influences economic expectations and outcomes. This would be the first causal identification in the sentiment literature from a developing economy.

\hspace*{0.5em}\textbf{Hypothesis 4 (Policy Relevance):} BENI-based nowcasts provide economically significant improvement over Bangladesh Bank's current forecasting framework, informing monetary policy decisions. A one-standard-deviation negative sentiment shock predicts 0.3--0.5 percentage point decline in inflation expectations within six weeks---actionable for a central bank with limited real-time data infrastructure.

\noindent\textbf{Data and Methods}: 8--10 major Bangla newspapers (Prothom Alo, The Daily Star, Dhaka Tribune, among others) from 2018--2024. Methods: BanglaBERT fine-tuned on economic text, topic modeling (keyATM) for narrative decomposition, dynamic factor model for index aggregation. Validation: lead-lag analysis, Granger causality, MIDAS regression, and comparison with Bangladesh Bank official statistics.

\noindent\textbf{Novelty}: First native-language economic sentiment index for any South Asian language. First causal identification attempt in a non-OECD sentiment context. Direct test of the information-versus-noise hypothesis through cross-language comparison.

\subsection{Required Infrastructure}

Executing this program requires: (1) digitized news corpora from major Bangla newspapers with consistent formatting, (2) domain-specific financial lexicons in Bangla, currently unavailable and requiring manual construction or automated extraction from domain corpora, (3) native transformer fine-tuning on economic text, leveraging BanglaBERT and MuRIL multilingual models, and (4) validation against Bangladesh's quarterly GDP (released with 2--3 month lag), monthly CPI, and daily exchange rate data.

\section{A 10-Year Research Agenda}

The BENI blueprint reveals that the three tensions are addressable through coordinated research investment. We propose a phased 10-year agenda:

\subsection*{Phase I: Geographic Expansion and Causal Identification (Years 1--3)}

\noindent\textbf{Primary goal}: Expand sentiment indices to 5--10 underrepresented economies using standardized methodology.

\noindent\textbf{Activities}:
\begin{itemize}
    \item Construct BENI (Year 1) with testable hypotheses H1--H4
    \item Replicate for India (Hindi, Tamil, Bengali), Brazil (Portuguese), Nigeria (Hausa, English), Indonesia (Bahasa Indonesia) using local-language models
    \item Identify natural experiments in each market: ownership changes, newspaper strikes, platform shutdowns, regulatory changes affecting media coverage
    \item Run IV and DiD specifications for each experiment
\end{itemize}

\noindent\textbf{Milestones}: BENI operational (Year 1); 5 cross-country causal estimates (Year 2); first cross-country comparison of information vs noise across markets (Year 3).

\subsection*{Phase II: Theoretical Integration (Years 4--7)}

\noindent\textbf{Primary goal}: Embed narrative dynamics into structural economic models.

\noindent\textbf{Activities}:
\begin{itemize}
    \item Develop DSGE models with sentiment shocks as explicit state variables (not just error terms)
    \item Agent-based models with narrative contagion dynamics (epidemiological spreading of narratives through social networks)
    \item Information diffusion networks linking media $\to$ public $\to$ policy $\to$ economic behavior, estimating elasticities at each node
    \item Micro-foundations: experiments measuring how individuals form and update economic narratives in response to news
\end{itemize}

\noindent\textbf{Milestones}: DSGE-with-sentiment working model (Year 5); experimental evidence on narrative formation (Year 6); integrated macro-model with endogenous narrative dynamics (Year 7).

\subsection*{Phase III: Production Systems (Years 8--10)}

\noindent\textbf{Primary goal}: Transition from research to operational central bank systems.

\noindent\textbf{Activities}:
\begin{itemize}
    \item Deploy BENI at Bangladesh Bank as production nowcasting system
    \item LLM-based narrative decomposition at scale (real-time processing of 100k+ articles/day)
    \item Multi-country sentiment dashboard for comparative policy analysis
    \item Standardized benchmark dataset for comparing sentiment methods across languages and economic tasks
\end{itemize}

\noindent\textbf{Milestones}: Central bank adoption (Year 8); production LLM system (Year 9); public benchmark dataset establishing industry standard (Year 10).

\noindent\textbf{Key dependencies}: NLP tool availability for target languages (currently strong for Bangla, Hindi, Portuguese; weaker for Hausa and Indonesian), central bank partnerships for data access and validation, computing infrastructure for transformer fine-tuning in resource-constrained settings.

\section{Conclusion}

This review has organized 18 years of sentiment-based economic research around three unresolved tensions---causality, information versus noise, and external validity---and shown that each is empirically consequential. The literature has robustly established that sentiment predicts economic outcomes (median 20\% RMSE improvement), but has not established whether sentiment causes them, whether the predictive information is fundamental or transient, or whether findings generalize beyond US English text.

We have formalized the identification problem (Section~3.4), showing that the condition $\mathbb{E}[\varepsilon_{t+h} \mid S_t, X_t] \neq 0$ is violated in every reviewed study, and proposed IV, DiD, and RDD strategies for resolution. We have shown that the information-versus-noise debate---Tetlock's reversal versus Da, Engelberg, and Gao's delayed recovery---remains unresolved because both interpretations fit the same data pattern; cross-market comparisons using the geographic expansion agenda can distinguish them.

The Bangla Economic Narrative Index provides a concrete blueprint for addressing all three tensions simultaneously. Its four testable hypotheses (H1--H4) span predictive replication, method comparison, causal identification, and policy relevance. The 10-year agenda---Phase I geographic expansion, Phase II theoretical integration, Phase III production systems---provides a roadmap from where the field stands today to where economists need it to be.

The next decade will determine whether sentiment-based economic analysis addresses these tensions or remains a promising but unfulfilled research program. The field has demonstrated empirical success in forecasting. It has not demonstrated scientific maturity. A coordinated research program---centered on causal identification, geographic expansion, and theoretical integration---can close this gap. If it does, sentiment analysis will transform from a forecasting tool into a core component of economic science.
